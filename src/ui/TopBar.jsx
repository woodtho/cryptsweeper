...
export function TopBar({ children, combatQuickStats = null }) {
  ...
  return (
    <div className={`topbar-shell ${combat ? 'combat-topbar-shell' : ''}`}>
      <div className="topbar-menu-row">
        <RunTimer />
        {combat && (
          <span className={`stat hpbar combat-quick-health ${critical ? 'critical' : ''}`} data-mechanic="health" tabIndex="0"
            aria-label={`Health ${run.hp} of ${run.maxHp}${critical ? ', critical' : ''}`}>
      ...
      {!combat && <div className="topbar">
        ...
        {run.gadgets.length > 0 && ui.screen !== 'combat' && (
          <span className="stat dim hud-secondary gadget-icons">Consumables: {run.gadgets.map(g => <i key={g}>{itemVector(g, prefs)}</i>)}</span>
        )}
        {children}
      </div>}
    </div>
